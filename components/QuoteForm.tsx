...
export default function QuoteForm({ onNext, onPrevious, initialData = {} }: QuoteFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('🔍 [QUOTEFORM] handleSubmit appelé');
    console.log('📋 [QUOTEFORM] Données actuelles:', formData);
    console.log('🎯 [QUOTEFORM] onNext est défini:', typeof onNext === 'function');
    
    const isValid = validateForm();
    console.log('✅ [QUOTEFORM] Formulaire valide:', isValid);
    console.log('❌ [QUOTEFORM] Erreurs détectées:', errors);
    
    if (isValid) {
      console.log('🚀 [QUOTEFORM] Appel de onNext avec les données...');
      try {
        onNext(formData);
        console.log('✅ [QUOTEFORM] onNext appelé avec succès');
      } catch (error) {
        console.error('❌ [QUOTEFORM] Erreur lors de l\'appel onNext:', error);
      }
    } else {
      console.log('🛑 [QUOTEFORM] Formulaire invalide - soumission bloquée');
      console.log('📝 [QUOTEFORM] Champs manquants:', Object.keys(errors));
    }
  };
